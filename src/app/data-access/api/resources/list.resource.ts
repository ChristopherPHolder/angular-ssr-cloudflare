...
const URL_ADD_MOVIE_TO_LIST = (id: number) => [URL_EXISTING_LIST(id), 'items'].join('/');
...
export class ListResource {
  ...
  createList = (params: TMDBListCreateUpdateParams): Observable<number> =>
    this.http.post<ListCreateResponse>(URL_LIST_BASE, params).pipe(map(({ id }) => id));

  fetchList = (id: string): Observable<Record<string, TMDBListModel>> =>
    this.http.get<TMDBListModel>(URL_EXISTING_LIST(+id)).pipe(map((list) => ({ [id]: list })));
